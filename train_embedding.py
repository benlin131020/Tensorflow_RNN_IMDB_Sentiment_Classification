...
def dynamicRNN(x, seqlen, weights, biases, seq_max_len):
    ...
    return tf.nn.sigmoid(tf.matmul(outputs, weights['out']) + biases['out'])
...
if __name__ == "__main__":
    ...
    with tf.name_scope("InputLayer"):
        ...
        x = tf.placeholder(tf.int32, [None, seq_max_len, 1], name='x')
        y = tf.placeholder(tf.int32, [None, NUM_CLASSES], name='y')
        # A placeholder for indicating each sequence length
        seqlen = tf.placeholder(tf.int32, [None], name='SeqLen')
    with tf.name_scope("Embedding"):
        word_embeddings = tf.get_variable("word_embeddings", [seq_max_len, EMBEDDING_SIZE])
        x_embed = tf.nn.embedding_lookup(word_embeddings, x)
        x_embed =tf.reshape(x_embed, [-1, seq_max_len, EMBEDDING_SIZE])
    with tf.name_scope("DynamicRNN"):
        pred = dynamicRNN(x_embed, seqlen, weights, biases, seq_max_len)
